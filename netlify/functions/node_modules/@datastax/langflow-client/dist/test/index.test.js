    ...
    describe("with a DataStax API URL", () => {
        ...
        describe("request", () => {
            it("makes a request to the baseURL with the full path to the method", async () => {
                const fetcher = createMockFetch({ session_id: "session-id", outputs: [] }, (input, init) => {
            ...
            it("includes a user agent in the headers", async () => {
                const fetcher = createMockFetch({ session_id: "session-id", outputs: [] }, (input, init) => {
                    ...
                    assert.ok(userAgent);
                    assert.match(userAgent, /^@datastax\/langflow-client\/\d+\.\d+\.\d+/);
                });
                const client = new LangflowClient({
                    baseUrl,
                    langflowId,
            ...
            it("throws a DOMException AbortError if the request is aborted", async () => {
                const fetcher = createMockFetch({ session_id: "session-id", outputs: [] }, () => {
    ...
    describe("with a custom API URL", () => {
        ...
        describe("stream", () => {
            ...
            it("makes a streaming request with the stream parameter", async () => {
                ...
                const fetcher = createMockFetch(events, (input, init) => {
                    ...
                    ok: true,
                    body: ReadableStream.from(events.map((e) => JSON.stringify(e))).pipeThrough(new TextEncoderStream()),
                });
                const client = new LangflowClient({
                    baseUrl,
                    fetch: fetcher,
            ...
            it("includes the API key in x-api-key header for streams", async () => {
                const fetcher = createMockFetch({}, (input, init) => {
                    const headers = new Headers(init?.headers);
                    assert.equal(headers.get("x-api-key"), apiKey);
                }, {
                    ok: true,
